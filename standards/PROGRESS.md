...
6. **CD `docker run` 报 `port is already allocated`(exit 125)**:主机 8000 被占用。
   解决:容器内端口固定 8000,**主机端口优先 8000、被占用就在 8000-8010 自动回退**(见 cd.yml),并把预留区间在服务器防火墙放行。`docker rm -f banksys` 保证自身可幂等替换。
